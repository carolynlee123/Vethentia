...
public class SkyFloatingLabelTextFieldWithIcon: SkyFloatingLabelTextField {
    ...
    @IBInspectable
    public var iconMarginBottom:CGFloat = 0 {
        didSet {
            self.updateFrame()
        }
    }
    ...
    */
    ...
    private func updateFrame() {
        ...
        if isLTRLanguage {
            self.iconLabel.frame = CGRectMake(0, self.bounds.size.height - textHeight + 2, iconWidth, textHeight)
        } else {
            self.iconLabel.frame = CGRect(x: textWidth - iconWidth , y: self.bounds.size.height - textHeight - 5, width: iconWidth, height: textHeight)
        }
    }
}
